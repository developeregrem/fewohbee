...
/**
 * Token-authenticated iCal export. Unlike the public UUID route, this one does not
 * require the calendar to be marked public — access is granted via API token scope.
 */
class ApartmentCalendarApiController extends AbstractController
{
    ...
    public function ics(int $id, ManagerRegistry $doctrine, CalendarService $cs, CalendarSyncService $css): Response
    {
        $em = $doctrine->getManager();
        $apartment = $em->getRepository(Appartment::class)->find($id);
        if (!$apartment instanceof Appartment || !$apartment->isActive()) {
            throw new NotFoundHttpException();
        }
        $sync = $em->getRepository(CalendarSync::class)->findOneBy(['apartment' => $apartment]);
